...
`cd #{path}/; ruby app.rb port=3000 config=#{parser.value('config')} > /dev/null 2>&1 &`
# Final message
puts
puts "Niginx is running.\nWait a few seconds for Sinatra to be running too.\nIf it doesn't work, try the command `startf.rb` (stat in foreground mode) to see command outputs." 
