tagsJs = {
    ...
    //
    set_color: function(parent, s) {
        let id_lead = parent.data('id');
        // get the span.caption inside the button.btn-tag-lists with data-id='id_lead'
        // get the value of the span
        // update the text into the span, with the number of tags with checked='true'
        let i = $('button.btn-tag-lists[data-id="'+id_lead+'"] span i');
        i.css('color', s);
    },

    //
    set_background_color: function(parent, s) {
        let id_lead = parent.data('id');
        // get the span.caption inside the button.btn-tag-lists with data-id='id_lead'
        // get the value of the span
        // update the text into the span, with the number of tags with checked='true'
        let i = $('button.btn-tag-lists[data-id="'+id_lead+'"] span.badge-main');
    ...
    is_tag_exist: function(parent, h) {
        let id_lead = parent.data('id');
        // Try to find the existing tag by its id
        let existingTag = document.querySelector('li[data-id="'+id_lead+'"][data-id-tag-list="'+h.id+'"]');
    ...
    add_tag: function(parent, h) {
        ...
        if (h.checked) li.setAttribute('data-checked', h.checked.toString());
    ...
    update_tag: function(parent, h) {
        let id_lead = parent.data('id');
        let li = document.querySelector('li[data-id="'+id_lead+'"][data-id-tag-list="'+h.id+'"]');
        
        if (li) {
            // Update tag attributes
            li.setAttribute('data-name', h.name);
            
            // Update the icon (checked/unchecked state)
            let icon = li.querySelector('i');
            icon.classList.remove('icon-check', 'icon-check-empty');
            icon.classList.add(h.checked ? 'icon-check' : 'icon-check-empty');
            icon.style.color = h.checked ? 'green' : 'gray'; // Apply color directly
            
            // Update tag color and name
            let badge = li.querySelector('span.badge-black');
            let badgeIcon = badge.querySelector('i');
            badgeIcon.style.color = h.color; // Update the color
            badge.childNodes[1].nodeValue = ' ' + h.name; // Update the name
            
            // Handle the "deleted" state
            if (h.deleted) {
                if (!li.querySelector('.deleted-text')) {
                    let deletedText = document.createElement('span');
                    deletedText.classList.add('deleted-text');
                    deletedText.textContent = ' (deleted)';
                    li.appendChild(deletedText);
                }
            } else {
                let deletedText = li.querySelector('.deleted-text');
                if (deletedText) deletedText.remove();
            }
        }
    },    
    
    add_or_update_tag: function(parent, h) {
        // Remove the 'No tag lists found' message if it exists
        $('i:contains("No tag lists found")').remove();
        
        // Try to find an existing tag
        let existingTag = tagsJs.is_tag_exist(parent, h);
    
        // If the tag exists, update it
        if (existingTag) {
            tagsJs.update_tag(parent, h);
        } else {
            // If the tag does not exist, add it
            tagsJs.add_tag(parent, h);
        }
    },
    ...
    draw: function(parent, h) {
        ...
        button_border = false
        if (h['button']) button_border = h['button'];

        color = 'black';
        if (h['color']) color = h['color'];

        background_color = 'black';
        if (h['background_color']) background_color = h['background_color'];

        let tagsHtml = `
            <div class="tags" data-id="${id_lead}">
                <div class="buttons">
                    <div class="btn-group" data-id="${id_lead}">
                        <button class="btn btn-${button_border ? 'black' : 'link'} dropdown-toggle btn-tag-lists" data-id="${id_lead}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            <span class="badge badge-main" style="background-color:${background_color}"><i class='icon-circle' style='color:${color};'></i> <span class='caption '>${text}<span></span>
                        </button>
