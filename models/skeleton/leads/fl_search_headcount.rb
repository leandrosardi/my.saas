module BlackStack
module Leads
  class SearchHeadcount < Sequel::Model(:fl_search_headcount)
    ...
    def to_hash
      ...
      }.merge(self.fl_headcount.to_hash)
    end
    ...
    def self.validate_descriptor(h)
      ...
      if h.is_a?(Hash)
        ...
        errors << "Key 'name' is mandatory for BlackStack::Leads::SearchHeadcount" if !h.has_key?('name')
        ...
        if h.has_key?('name') && h['name'].to_s.length > 0
          industry = BlackStack::Leads::Headcount.where(:name=>h['name']).first
          errors << "Value 'name' must exists in the table fl_headcount" if industry.nil?
        end
    ...
    def update(h)
      self.id_headcount = h['name'].to_s.length > 0 ? BlackStack::Leads::Headcount.where(:name=>h['name']).first.id : nil
  ...
  end # BlackStack::Leads::SearchHeadcount
end # Leads
end